...
const MULTI_CHAR_TOKENS: [string, TokenKind][] = [
  ["+=", "PlusEqual"],
  ["-=", "MinusEqual"],
  ["*=", "StarEqual"],
  ["/=", "SlashEqual"],
...
export class Lexer {
  ...
  constructor(private source: string, startLine = 1, startCol = 1) {
    this.line = startLine;
  ...
  private scanInterpString(): string {
    const startLine = this.line;
    const startCol = this.column;
    let value = "";
  ...
  private scanIdentifier(start: string): string {
    let ident = start;
