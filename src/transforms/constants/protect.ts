...
export class ConstantProtectionTransform {
  ...
  private transformStatement(stmt: Statement, context: TransformContext): Statement {
    ...
    switch (stmt.kind) {
      ...
      case "CompoundAssignment":
        return { ...stmt, right: this.transformExpression(stmt.right, context) } as any;
      case "ReturnStatement":
        return { ...stmt, expressions: stmt.expressions.map(e => this.transformExpression(e, context)) };
      case "CallStatement":
        return { ...stmt, expression: this.transformExpression(stmt.expression as any, context) as any };
      ...
      case "ExportDeclaration":
        return { ...stmt, declaration: this.transformStatement(stmt.declaration, context) as any };
      default:
        return stmt;
    }
  }

  private transformExpression(expr: Expression, context: TransformContext): Expression {
  ...
  private protectNumber(num: NumberLiteral, context: TransformContext): Expression {
    const value = parseFloat(num.raw);
    if (isNaN(value)) return num;
    if (value === 0 || value === 1 || value === -1) return num;
  ...
  private protectBoolean(bool: BooleanLiteral, context: TransformContext): Expression {
    if (bool.value) {
