import type { CompilationResult, CompiledProto } from "../vm.js";
import type { TransformContext } from "../../transform.js";
import type { VMProfile, HandlerVars, OpTables, OpcodeMappingResult } from "./base.js";
import { BINARY_OP_CODES, UNARY_OP_CODES } from "./base.js";
import {
  rndName, luaStr, luaVal, ALL_OPS,
  makeOpTables, makeVars, handlerBody,
  generateBit32Fallback, generateNamesTable, generateProtosTable,
  generateSplitConstantPools, generateShuffledConstantPool,
  generateOpcodeMapping,
  makeRegisterVars, registerHandlerBody, createSemEncoder, generateEnvInit,
} from "./base.js";
...
function remapOps(protos: CompiledProto[], tbl: OpTables): void {
  for (const proto of protos) {
    for (const instr of proto.instructions as any[]) {
      if (Array.isArray(instr) && instr[0] === "raw") continue;
      if (instr[0] === OP.Binary) {
        const sym = Object.keys(BINARY_OP_CODES).find(k => BINARY_OP_CODES[k as any] === instr[1]);
        if (sym) instr[1] = tbl.symbolToBinCode[sym];
      }
      if (instr[0] === OP.Unary) {
        const sym = Object.keys(UNARY_OP_CODES).find(k => UNARY_OP_CODES[k as any] === instr[1]);
        if (sym) instr[1] = tbl.symbolToUnaryCode[sym];
      }
    }
  }
}

function emitBit32(protos: CompiledProto[], parts: string[], envName: string, globals: string[]): void {
  let needsBit32 = false;
  for (const proto of protos) {
    for (const instr of proto.instructions as any[]) {
      if (Array.isArray(instr) && instr[0] === "raw") continue;
      if (instr[0] === OP.Binary && Object.keys(BINARY_OP_CODES).find(k => BINARY_OP_CODES[k as any] === instr[1+0])?.match(/[&|~]/)) needsBit32 = true;
    }
  }
  parts.push(generateBit32Fallback());
}

export class ProfileA implements VMProfile {
  name = "profileA";
  ...
  generateRuntime(result: CompilationResult, context: TransformContext): string {
    ...
    const handlerFuncs: string[] = [];
    const handlerTableEntries: string[] = [];
  ...
  ${body}
    ...
    for (const line of generateEnvInit(globals, envName, namesName, decoderName)) parts.push(line);
    ...
    for (const hf of handlerFuncs) parts.push(hf);
...
export class ProfileB implements VMProfile {
  name = "profileB";
...
export class ProfileC implements VMProfile {
  name = "profileC";
...
export class ProfileD implements VMProfile {
  name = "profileD";
  ...
  generateRuntime(result: CompilationResult, context: TransformContext): string {
    ...
    const allOpsShuffled = r.shuffle([...ALL_OPS]);
    const inlineOps: number[] = [];
    const tableOps: number[] = [];
...
// ───────────────────────────────────────────────────────────────────
export class ProfileE implements VMProfile {
  name = "profileE";
  ...
  generateRuntime(result: CompilationResult, context: TransformContext): string {
    ...
    const revMap = new Map<number, number>();
    for (const op of ALL_OPS) revMap.set(mapping.encodeValue(op), op);
    const revEntries: string[] = [];
    for (const [encOp, orig] of revMap.entries()) revEntries.push(`[${encOp}]=${orig}`);
    ...
    for (const line of generateEnvInit(globals, env, v.names, context.stringPoolDecoderName)) parts.push(line);
