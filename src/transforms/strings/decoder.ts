...
export function generatePortableXorFunction(fnName: string): string {
  return (
...
export function generateXorDecoderBody(
  decoderName: string,
  xorFnName: string,
  keyExpr: string,
  cacheName: string,
...
export function generateRotateDecoderBody(
  decoderName: string,
  offset: number,
  cacheName: string,
  poolName: string,
...
export function generateXorChunkedDecoderBody(
  decoderName: string,
  xorFnName: string,
  chunkSize: number,
...
export function dedupeEmptyLocalTables(code: string): string {
  const seen = new Set<string>();
  return code.replace(/^local\s+([A-Za-z_][\w]*)\s*=\s*\{\}\s*$/gm, (full, name: string) => {
