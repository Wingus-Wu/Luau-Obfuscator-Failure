...
function makeStr(value: string, line = 0, column = 0): StringLiteral {
...
function makeMember(object: Expression, property: string, line = 0, column = 0): MemberExpression {
...
export class AntiTamperTransform {
  ...
  apply(ast: Program, context: TransformContext): Program {
    ...
    const checks: Statement[] = [];
    const protectedSections: { name: string; hash: number; data: string }[] = [];

    // Collect functions to protect
    const functionsToProtect: FunctionDeclaration[] = [];
    ...
    const verifyFunc: FunctionDeclaration = {
      kind: "FunctionDeclaration",
  ...
  private serializeFunction(func: FunctionDeclaration): string {
