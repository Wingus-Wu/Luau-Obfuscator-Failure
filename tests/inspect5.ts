...
function runLua(source: string) {
  ...
  if (loadResult !== 0) return { stdout: "", error: to_jsstring(lua.lua_tostring(L, -1)) };
  const pcallResult = lua.lua_pcall(L, 0, 0, 0);
  if (pcallResult !== 0) return { stdout: stdout.join("\n"), error: to_jsstring(lua.lua_tostring(L, -1)) };
