    ...
    visitStatement(stmt) {
        switch (stmt.kind) {
            ...
            case "FunctionDeclaration": {
                ...
                fd += this.visitExpression(stmt.name);
            ...
            case "Assignment": {
                ...
                for (let i = 0; i < stmt.left.length; i++) {
                    if (i > 0)
                        asn += ", ";
                    asn += this.visitExpression(stmt.left[i]);
                }
                asn += " = ";
                for (let i = 0; i < stmt.right.length; i++) {
                    if (i > 0)
            ...
            case "CompoundAssignment": {
                ...
                const left = this.visitExpression(stmt.left);
            ...
            case "CallStatement":
                return "  ".repeat(this.indent) + this.visitExpression(stmt.expression) + ";";
            ...
            case "NumericForLoop": {
                ...
                if (stmt.step)
                    nfl += ", " + this.visitExpression(stmt.step);
                nfl += " do";
                for (const s of stmt.body) {
                    nfl += "\n" + this.visitStatement(s);
                }
    ...
    visitParams(params) {
        ...
        for (const p of params) {
            if (p.isVararg) {
                ...
                let param = p.name;
                if (p.typeAnnotation)
                    param += ": " + p.typeAnnotation;
                parts.push(param);
            }
        }
        return "(" + parts.join(", ") + ")";
    }
    visitInterpString(expr) {
        let result = "";
