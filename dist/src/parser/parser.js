...
export class ParseError extends Error {
    constructor(token, message, source) {
...
export class Parser {
    ...
    parseStatement() {
        ...
        if (this.match("Export")) {
            if (this.match("Type"))
                return this.parseExportDeclaration(this.parseTypeExport());
            if (this.match("Function"))
                return this.parseExportDeclaration(this.parseFunctionDeclaration(false));
            return this.parseExportDeclaration(this.parseLocalDeclaration());
        }
        if (this.match("Type"))
    ...
    parseLocalDeclaration() {
        ...
        if (this.match("Assign")) {
            while (!this.isAtEnd() && !this.check("Semicolon")) {
                if (exprs.length > 0 && !this.check("Comma"))
                    break;
                if (exprs.length > 0)
                    this.consume("Comma", "Expected ',' in variable declaration");
    ...
    parseFunctionDeclaration(isMethod) {
        const token = this.previous();
        const name = this.identifier();
        const params = this.parseParams();
        const returnAnnotations = this.parseReturnAnnotation();
    ...
    parseTypeParams() {
        ...
        while (!this.check("Greater") && !this.isAtEnd()) {
            if (params.length > 0)
                this.consume("Comma", "Expected ',' in type parameters");
        ...
        while (!this.check("RParen") && !this.isAtEnd()) {
            ...
            if (this.match("Colon")) {
                ...
                while (!this.isAtEnd() && (depth > 0 || (!this.check("Comma") && !this.check("RParen")))) {
                    if (this.check("LParen") || this.check("Less"))
                        depth++;
                    else if (this.check("RParen") || this.check("Greater"))
                        depth--;
                    if (depth === 0 && (this.check("Comma") && this.peek(1).kind !== "Vararg"))
                        break;
        ...
        if (this.match("Assign")) {
            ...
            while (!this.check("Do") && !this.isAtEnd()) {
                if (expressions.length > 0)
                    this.consume("Comma", "Expected ',' in generic for");
    ...
    parseReturnStatement() {
        ...
        while (!this.isAtEnd() && !stopKinds.has(this.peek().kind)) {
            if (exprs.length > 0 && !this.check("Comma"))
                break;
            if (exprs.length > 0)
                this.consume("Comma", "Expected ',' in return statement");
            exprs.push(this.parseExpression());
        }
        this.maybeSemicolon();
    ...
    parseEquality() {
        ...
        while (this.match("EqualEqual", "TildeEqual")) {
            ...
            left = { kind: "BinaryExpression", operator: token.value, left, right, ...loc(token) };
        }
        return left;
    }
    parseComparison() {
    ...
    parseUnary() {
        if (this.match("Not", "Minus", "Hash", "Tilde", "Typeof")) {
            ...
            return { kind: "UnaryExpression", operator: token.value, argument: arg, ...loc(token) };
        }
    ...
    parseExpressionFromText(text, line, col) {
    ...
    parsePostfix(expr) {
        while (this.check("Colon") || this.check("LBracket") || this.check("Dot") || this.check("LParen") || this.check("LBrace")) {
            if (this.match("Colon")) {
                ...
                expr = this.finishCall(expr, this.previous());
            }
        }
        return expr;
    }
    finishCall(callee, token) {
        const args = this.parseArguments();
    ...
    parseArguments() {
        ...
        if (this.match("LParen")) {
            while (!this.check("RParen") && !this.isAtEnd()) {
                if (args.length > 0)
                    this.consume("Comma", "Expected ',' between arguments");
    ...
    parseFunctionExpression(isMethod) {
        const token = this.previous();
        const params = this.parseParams();
        const returnAnnotations = this.parseReturnAnnotation();
        const body = this.parseBlock();
