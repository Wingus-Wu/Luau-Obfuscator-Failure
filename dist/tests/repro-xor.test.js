...
local ratio = 3.14
`;
function findBadTokens(out) {
    const bad = [];
    // binary ~ (not ~=, not unary before digit/paren-id)
    for (const m of out.matchAll(/[0-9)\]\}](~)\(/g))
        bad.push("binary-~(at:" + m.index + ")");
    if (out.match(/\b0x[0-9a-fA-F]+\b/))
        bad.push("hex-literal");
    return bad;
}
    ...
    it("default config (all transforms) stays valid across seeds", () => {
        ...
        for (let i = 0; i < 300; i++) {
            ...
            if (bad.length) {
                badTokenHits++;
                if (badTokenHits <= 5)
                    console.log("BADTOKEN", bad, "on full-" + i);
            }
        }
    ...
    it("constantProtection only (expr off) stays valid across seeds", () => {
        ...
        for (let i = 0; i < 300; i++) {
            ...
            if (bad.length) {
                badTokenHits++;
                if (badTokenHits <= 5)
                    console.log("BADTOKEN", bad, "on cp-" + i);
            }
        }
